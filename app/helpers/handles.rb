module Handles
  PolHandles = ["HillaryClinton", "SenSanders", "realDonaldTrump", "JimWebbUSA", "RealBenCarson", "ChrisChristie", "JoeBiden", "BarackObama", "PRyan",
                                "SarahPalinUSA", "SpeakerBoehner", "newtgingrich", "GovGaryJohnson", "THEHermanCain", "sen.franken", "Schwarzenegger", "nicolassarkozy", 
                                "georgewbush", "algore", "ronpaul", "DWStweets", "EricCantor", "CarlyFiorina", "JebBush", "marcorubio", "DrJillStein", "JohnKasich",
                                "GovWalker", "JoeLieberman", "BachusAL06", "PeteSessions", "RepBillShuster", "WhipHoyer",
                                "DarrellIssa", "cbrangel", "RepMarryFallin", "SenJohnMcCain", "DanaRohrabacher", "DavidVitter", 
                                "OrrinHatch", "lisamurkowski", "SenatorReid", "russfeingold", "Dennis_Kucinich", "JimDeMint"]
                                
  CelHandles = ["TheRealJRSmith", "JoelEmbiid", "n3twork", "ZachLowe_NBA", "BillSimmons", "AminESPN", "NateDuncanNBA", "HPbasketball", 
                               "HickoryHigh", "SethPartnow", "SherwoodStrauss", "teamziller", "Pflanns", "treykerby", "LeighEllis", "TasMelas",
                               "jeskeets", "talkhoops", "MagicJohnson", "Barryathree", "blakegriffin32", "NickSwagyPYoung", "Dloading", "shaq",
                               "russellwestbrook", "dirknowitski", "mister6clutch", "bobbysacre", "Scalabrine", "RedMamba_15", "SeinfeldToday", "TurnerSportsEJ",
                               "TheJetOnTNT", "RealStevenAdams", "arronafflalo", "colea45", "aldridge_12", "greenRAYn20", "aa000G9",
                               "Adeto_kouboS", "agentzeroshow", "Gustavo_Ayon15", "ShaneBattier", "kobebryant", "mchalmers15", "boogiecousins",
                               "StephenCurry30", "KDTrey5", "manuginobili", "Money23Green", "Jrue_Holiday11", "DwightHoward", 
                               "Reggie_Jackson", "KingJames", "ZachLaVine", "kawhileonard", "Dame_Lillard", "tp9network",
                               "ChandlerParsons", "AustinRivers25", "drose", "rickyrubio9", "KarlTowns", "KembaWalker", "DwyaneWade", 
                               "youngwhiteside", "22wiggins", "IAmJustise", "MettaWorldPeace"]
                               
  HashTags = ["#America", "#4Life", "#politiciansofinstagram", "#keepit100", "#SwishinAndDishin", "#SurgingAndSplurging", "#LegislateFromTheBench",
                                "#Freedom", "#Patriot", "#GunsAndButter", "#donateplz", "#corporationsunited", "#boardwalk", "#payup", "#SellinToChina",
                                "#DeficitsDontMatter", "#YoureAfiliBusta", "#SlamDunkLegislation", "#ClotureDefense", "#6thSenatorOTY", "#GOAT", "#SphericalOffice",
                                "#OutofBounds", "#CallYourFouls", "#DoYouEvenTriangle?", "#ProChoiceTheProcess", "#RepresentativeOTY", "#GoalTending", "#pantsONFIRE",
                                "#YES", "#BANG!", "#FromCapitalHill", "#babyfacedassassin", "#capitalGAINSfromtheGYM", "#EmailsFromErdogan", "#DYNAMITE", "#trainingday",
                                "#fromDOWNTOWN", "#BOOMSHAKALAKA", "#JamsItIn", "#justabitoutside", "#RazzleDazzle", "#ImOnFire", "#KidAisAOkay", "#thaichilipeppersare100",
                                "#TalesFromTheHill", "#stayshook", "#preach", "#iamkill", "#allyouneediskill", "#WhoseAllyIsSaudiArabiaAnyway", "#stepbacklegislation",
                                "#r/nba", "#SueMe", "#KStreetKings", "#AnalyzeThis", "#DeathTax", "#RegressiveTaxPolicy", "#Containment", "#ForeignAid", "#Outreach",
                                "#DEMOCRAZY", "#RepubliCanCan", "#2Incumbent2Quit"]
end